...
let MongoDB = (exports.MongoDB = function (options) {
  ...
  function setupDatabaseAndEmptyQueue(db) {
    return createCollection(db).then(
      (db) => {
        self.logDb = db;
        processOpQueue();
      },
  ...
  function createCollection(db) {
    ...
    return db
      .createCollection(self.collection, opts)
      .then((col) => {
        ...
        return col.indexInformation({ full: true }).then((info) => {
          ...
          if (info.length === 0) {
            ...
            if (
              info[0].expireAfterSeconds !== undefined &&
              info[0].expireAfterSeconds !== self.expireAfterSeconds
            ) {
              return col
                .dropIndex(ttlIndexName)
  ...
  function connectToDatabase(logger) {
    return mongodb.MongoClient.connect(logger.db, logger.options).then(
      (client) => {
  ...
  if ("string" === typeof this.db) {
    ...
    this.db.then(
      ...
      (err) =>
        console.error(
          "winston-mongodb: error initialising logger from promise",
          err
        )
    );
...
MongoDB.prototype.close = function () {
  ...
  this.mongoClient
    .close()
    .then(() => (this.mongoClient = null))
    .catch((err) => {
      console.error(
        "Winston MongoDB transport encountered on error during " + "closing.",
        err
      );
    });
};
...
MongoDB.prototype.log = function (info, cb) {
  ...
  process.nextTick(() => {
    ...
    let entry = {
      timestamp: new Date(),
      level: this.decolorize
        ? info.level.replace(decolorizeRegex, "")
        : info.level,
    };
    let message = util.format(info.message, ...(info.splat || []));
    entry.message = this.decolorize
      ? message.replace(decolorizeRegex, "")
      : message;
